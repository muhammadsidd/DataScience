...
spark.sql(f"""DELETE DATABASE {user_db} """)
...
# df=df.withColumn("_error_ssn_format",f.when(f.col("sales_rep_ssn").contains('-'), True).otherwise(False))
# display(df)
...
df1 = spark.sql("""
select * from {}
""".format(batch_temp_view))
df2 = df1.withColumn('_error_ssn_format', F.when(F.col("sales_rep_ssn").rlike("[-]"), F.lit(True)).otherwise(F.lit(False)))
df3 = (df2.withColumn("sales_rep_ssn", F.regexp_replace("sales_rep_ssn", "-", ""))
       .withColumn("sales_rep_zip", F.col("sales_rep_zip").cast("int"))
       .withColumn("sales_rep_ssn", F.col("sales_rep_ssn").cast("long"))
       .drop("submitted_at")
       .drop("order_id")
       .drop("customer_id")
       .drop("shipping_address_attention", "shipping_address_address", "shipping_address_city", "shipping_address_state", "shipping_address_zip")
       .drop("product_id", "product_quantity", "product_sold_price")
      )
df4=df3.dropDuplicates(["sales_rep_id", "sales_rep_ssn", "sales_rep_first_name","sales_rep_last_name","sales_rep_last_name","sales_rep_city","sales_rep_state","_error_ssn_format"])
display(df4)
df4.write.format("delta").mode("overwrite").saveAsTable(sales_reps_table)
