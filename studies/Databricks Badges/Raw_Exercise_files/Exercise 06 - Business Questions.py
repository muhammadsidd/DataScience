...
df = spark.sql(f"""SELECT * FROM {orders_table} """)
# Use this cell to complete your solution
df = df.groupBy("shipping_address_state").count()
...
odf = spark.sql(f"""SELECT * FROM {orders_table} WHERE shipping_address_state = 'NC' """)
ldf = spark.sql(f"""SELECT * FROM {line_items_table} """)
pdf = spark.sql(f"""SELECT * FROM {products_table} WHERE color = 'green' """)
sdf = spark.sql(f"""SELECT * FROM {sales_reps_table} WHERE _error_ssn_format = True """)

# display(odf)
# display(ldf)
# display(pdf)
# display(sdf)
df = odf.join(sdf, "sales_rep_id", "right")
df2 = pdf.join(ldf,"product_id", "left")
maindf = df.join(df2, "order_id", "left")

mean = maindf.groupBy().avg("product_sold_price").take(1)[0][0]
minimum = maindf.groupBy().min("product_sold_price").take(1)[0][0]
maximum = maindf.groupBy().max("product_sold_price").take(1)[0][0]

maindf =  maindf.withColumn("avg(product_sold_price)", F.lit(mean)).withColumn("min(product_sold_price)",F.lit(minimum)).withColumn("max(product_sold_price)", F.lit(maximum))


# maindf.agg({"product_sold_price": "min"}).collect()
# maindf.agg({"product_sold_price": "max"}).collect()

maindf.createOrReplaceTempView(question_2_results_table)

display(maindf)

# display(maindf.count())
...
odf = spark.sql(f"""SELECT * FROM {orders_table} """)
ldf = spark.sql(f"""SELECT * FROM {line_items_table} """)
pdf = spark.sql(f"""SELECT * FROM {products_table} """)
sdf = spark.sql(f"""SELECT * FROM {sales_reps_table} """)

odf = odf.drop("customer_id", "shipping_address_attention","shipping_address_address","shipping_address_city","shipping_address_state","shipping_address_zip","ingest_file_name","ingested_at","submitted_yyyy_mm","sub,otted_at")
ldf = ldf.drop("ingest_file_name","ingested_at")
pdf = pdf.drop("model_name", "model_number","color","color_adj","size_adj","size")
sdf = sdf.drop("sales_rep_ssn", "_error_ssn_format","sales_rep_address","sales_rep_city","sales_rep_state","sales_rep_zip","ingest_file_name","ingested_at")

display(odf)
display(ldf)
display(pdf)
display(sdf)
df = odf.join(sdf, "sales_rep_id")
df2 = pdf.join(ldf,"product_id")
maindf = df.join(df2, "order_id")

def getProfit(baseprice,saleprice,quantity):
  return float(float(saleprice) - baseprice) * float(quantity)

profitUDF = udf(lambda x,y,z: getProfit(x,y,z), FloatType())

maindf = maindf.withColumn("profit", profitUDF(maindf.price,maindf.product_sold_price,maindf.product_quantity))

df1 = maindf.groupBy("sales_rep_id").agg(sum("profit").alias("sum(total_profit)"))
maindf =  maindf.join(df1,"sales_rep_id")
maindf = maindf.select("sales_rep_first_name","sales_rep_last_name","sum(total_profit)")
maindf = maindf.dropDuplicates()

# df2 =  maindf.groupby().agg(max("sum(total_profit)"))
# display(df2)

maximum = maindf.groupBy().max("sum(total_profit)").take(1)[0][0]
maindf = maindf.filter(col("sum(total_profit)") == maximum)
maindf.createOrReplaceTempView(question_3_results_table)
display(maindf)
