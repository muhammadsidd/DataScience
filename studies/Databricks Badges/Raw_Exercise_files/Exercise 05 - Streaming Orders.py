...
from pyspark.sql.functions import lit,col,substring,current_timestamp
schema = "customerId string, orderId string , salesRepId string ,shippingAddress STRUCT<address: STRING, attention: STRING, city: STRING, state: STRING ,zip: STRING>, submittedAt timestamp"
StreamReader = spark.readStream.schema(schema).options(maxFilesPerTrigger = 1).json(stream_path)
StreamReader = StreamReader.select("customerId","orderId","salesRepId","shippingAddress.address","shippingAddress.attention","shippingAddress.city","shippingAddress.state","shippingAddress.zip","submittedAt")
StreamReader = StreamReader.withColumn("submitted_yyyy_mm", substring('submittedAt', 1,7)).withColumn("ingest_file_name",lit(stream_path)).withColumn("ingested_at", lit(current_timestamp()))
StreamReader.createOrReplaceTempView("streamVeiw")
StreamReader = spark.sql("""select submittedAt as submitted_at,submitted_yyyy_mm,orderId as order_id,salesRepId as sales_rep_id
                            ,attention as shipping_address_attention,address as shipping_address_address,city as shipping_address_city
                           ,state as shipping_address_state, cast(zip as int) as shipping_address_zip,ingest_file_name,ingested_at from streamVeiw""")
#StreamReader.printSchema()
#StreamReader.display()
streamingQuery = (StreamReader.writeStream.outputMode("append").queryName(orders_table)
                  .option("checkpointLocation",orders_checkpoint_path)
                  .partitionBy("submitted_yyyy_mm").table(orders_table))

c = spark.sql(f"""SELECT COUNT(*) FROM {orders_table}""")
display(c)
print(c)
...
df =df.withColumn('product_id', F.col('exploded').getItem("productId")).withColumn('product_quantity', F.col('exploded').getItem("quantity")).withColumn('product_sold_price', F.col('exploded').getItem("soldPrice"))
...
df = df.drop("sales_rep_id")
df = df.drop("quantity")
display(df)
df.printSchema()

df.writeStream.format('delta').queryName(line_items_table).outputMode("append").option("checkpointLocation", line_items_checkpoint_path).table(line_items_table)

# root
#  |-- order_id: string (nullable = true)
#  |-- product_id: string (nullable = true)
#  |-- product_quantity: integer (nullable = true)
#  |-- product_sold_price: decimal(10,2) (nullable = true)
