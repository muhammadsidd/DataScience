...
df = spark.read.format("com.databricks.spark.xml").option("rootTag", "products").option("rowTag", "product").option("header",True).option("inferSchema" , True).load(products_xml_path)
df.printSchema()
display(df)

df = df.na.drop(subset=["price"])
df = df.withColumn("product_id",col("_product_id")).withColumn("base_price",col("price._base_price")).withColumn("color_adj",col("price._color_adj")).withColumn("size_adj",col("price._size_adj")).withColumn("price",col("price.usd"))
df = df.drop("_product_id")
df = df.select("product_id","color","model_name","model_number","base_price","color_adj","size_adj","price","size")
df.printSchema()
display(df)
df.show()
